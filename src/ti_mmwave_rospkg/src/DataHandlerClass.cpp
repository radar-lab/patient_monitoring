...
DataUARTHandler::DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {
    ...
    nh->getParam("/ti_mmwave/fc_chirp", fc_chirp);
...
void *DataUARTHandler::sortIncomingData( void )
{
    ...
    ti_mmwave_rospkg::RadarScan *radarscan_cf = NULL; // current frame
    ti_mmwave_rospkg::RadarScan *radarscan_lf = NULL; // last frame
    ti_mmwave_rospkg::RadarScan radarscan_pub; // last frame
    ...
    while(ros::ok())
    {
        ...
        case READ_POINT_CLOUD_2D:
            Point_List_Idx ++;
            //store the points in last frame
            //ROS_WARN("num_Points_lastframe = %d", num_Points_lastframe);
            //ROS_WARN("num_Points = %d", num_Points);
            if(radarscan_lf != NULL)
            {
                free(radarscan_lf);
                radarscan_lf = NULL;
            }
            if(num_Points_lastframe > 0)
            {
                radarscan_lf = (ti_mmwave_rospkg::RadarScan * )malloc(num_Points_lastframe * sizeof(ti_mmwave_rospkg::RadarScan));
            }

            if(radarscan_cf != NULL)
            {   
                memcpy(radarscan_lf, radarscan_cf, num_Points_lastframe * sizeof(ti_mmwave_rospkg::RadarScan));
                free(radarscan_cf);
                radarscan_cf = NULL;
            }
            if(radarscan_cf == NULL)
            {
                radarscan_cf = (ti_mmwave_rospkg::RadarScan * )malloc(num_Points * sizeof(ti_mmwave_rospkg::RadarScan));
            }
            ...
            for(i=0; i<num_Points; i++)
            {
                ...
                //get azimuth
                memcpy( &(radarscan_cf[i].bearing), &currentBufp->at(currentDatap), sizeof(mmwData.PointCloudData.angle));
                currentDatap += ( sizeof(mmwData.PointCloudData.angle) );
                //get doppler
                memcpy( &(radarscan_cf[i].velocity), &currentBufp->at(currentDatap), sizeof(mmwData.PointCloudData.doppler));
                currentDatap += ( sizeof(mmwData.PointCloudData.doppler) );
                //get snr
                memcpy( &(radarscan_cf[i].intensity), &currentBufp->at(currentDatap), sizeof(mmwData.PointCloudData.snr));
                currentDatap += ( sizeof(mmwData.PointCloudData.snr) );
                // radarscan_cf[i].intensity) = 10*(log(radarscan_cf[i].intensity));
                ...
                radarscan_cf[i].doppler_bin = round(radarscan_cf[i].velocity / dopplerRes + nd/2);
        ...
        case READ_TARGET_INDEX: 
            Point_List_Idx ++;
            num_TargetIdx = (tlvLen-sizeof(mmwData.tlvHeader))/sizeof(mmwData.TargetIndexData);
            for(i=0; i<num_TargetIdx; i++)
            {
                //get target index
                memcpy( &(radarscan_lf[i].target_idx), &currentBufp->at(currentDatap), sizeof(mmwData.TargetIndexData));
                currentDatap += ( sizeof(mmwData.TargetIndexData) );
            }
            if(Point_List_Idx == 3){
                for(i=0; i<num_TargetIdx; i++)
                {
                    radarscan_pub.header.frame_id       = "ti_mmwave_radar";
            	    radarscan_pub.header.stamp          = ros::Time::now();
                    radarscan_pub.point_id              = radarscan_lf[i].point_id;
                    radarscan_pub.target_idx            = radarscan_lf[i].target_idx;
                    radarscan_pub.x                     = radarscan_lf[i].x;
                    radarscan_pub.y                     = radarscan_lf[i].y;
                    radarscan_pub.z                     = radarscan_lf[i].z;
                    radarscan_pub.range                 = radarscan_lf[i].range;
                    radarscan_pub.velocity              = radarscan_lf[i].velocity;
               ...
               //get tlvType (32 bits) & remove from queue
                ...
                {                  
                    case MMWDEMO_OUTPUT_MSG_POINT_CLOUD_2D:
                        if( (tlvLen-sizeof(mmwData.tlvHeader))  % sizeof(mmwData.PointCloudData) == 0)
                        {
                    ...
                    case MMWDEMO_OUTPUT_MSG_TARGET_LIST_2D:
                        if( (tlvLen-sizeof(mmwData.tlvHeader))  % sizeof(mmwData.TargetListData) == 0)
                        {
                            num_Targets = (tlvLen-sizeof(mmwData.tlvHeader))/sizeof(mmwData.TargetListData);
...
void DataUARTHandler::visualize(const ti_mmwave_rospkg::RadarScan &msg){
    ...
    // ROS_WARN("Target Idx = %d", msg.target_idx);

    if(msg.target_idx < 253){
        marker.color.a = 1;
        marker.color.r = color_mat[msg.target_idx][1];
        marker.color.g = color_mat[msg.target_idx][2];
        marker.color.b = color_mat[msg.target_idx][3];
    }
    else{
        marker.color.a = 0;
        marker.color.r = 0;
